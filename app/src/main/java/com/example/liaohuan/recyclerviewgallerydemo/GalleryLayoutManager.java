...
//        LogUtil.e("mCurSelectedPosition " + mCurSelectedPosition);
    ...
    private void firstFillCover(RecyclerView.Recycler recycler, RecyclerView.State state, int scrollDelta) {
        ...
        if (mItemTransformer != null) {
            ...
            for (int i = 0; i < getChildCount(); i++) {
                child = getChildAt(i);
                mItemTransformer.transformItem(this, child, calculateToCenterFraction(child, scrollDelta),i);
            }
        }
...
//        if (Build.ID.toLowerCase().startsWith("dpc-pd1160")) {
            return Math.max(-3.f, Math.min(3.f, distance * 1.f / childLength));
//        } else
//            return Math.max(-1.f, Math.min(1.f, distance * 1.f / childLength));
    ...
    public interface ItemTransformer {
         ...
         */
        void transformItem(GalleryLayoutManager layoutManager, View item, float fraction, int position);
    }
